...
class ContrastiveSGL(nn.Module):
    def __init__(self, dim_in, dim_emb, device, gp_idx_list,
    ...
    def get_Lc_neg_mask(self, X, pair):
...
class UpdateBeta:
    def __init__(self, model, args):
        ...
        self.lam1 = args.L1
        self.lam2 = args.L21
        self.lam_Gau = args.Ls
        self.lam_con = args.Lc
        self.io = args.io
    ...
    def update_beta(self, X, Y, pair, t0_step=1.5):
        ...
        if (self.lam_con > 0) and (not stop_cond) and (i > 5):
            self.io.cprint('i access 5, stop majorization.')
            return True
        else:
            return False
    ...
    def get_Contrastive_grad(self, X, pair):
        ...
        loss_con = self.lam_con * self.model.get_Lc_neg_mask(X, pair)
        loss_con.backward()
    ...
    def validate_majorization(self, t, X, pair, grad_con, beta_prev):
        ...
        loss_con_orig = self.model.get_Lc_neg_mask(X, pair) * self.lam_con
        ...
        loss_con_majorized = self.model.get_Lc_neg_mask(X, pair) * self.lam_con\
