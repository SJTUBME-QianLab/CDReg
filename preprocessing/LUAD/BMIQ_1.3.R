...
require(RPMM); #nfit参数可改

BMIQ <- function(beta.v,design.v,nL=3,doH=TRUE,nfit=5000,th1.v=c(0.2,0.75),th2.v=NULL,niter=5,tol=0.001,plots=FALSE,sampleID=1){
...
Run_BMIQ <- function(data){
  n=dim(data)[2]
  samples_name=colnames(data[,1:n-1])
  design.v=data[,'type']
  normalized_result=sapply(samples_name, function(col){
  beta.v=data[,col]
  result=BMIQ(beta.v,design.v,nfit=5000,sampleID = col)
  result$nbeta
  })
  row.names(normalized_result)=row.names(data)
  as.data.frame(normalized_result)
}
...
R=Run_BMIQ(data)
write.table(R,file=outfile,sep="\t",quote=FALSE)
